...
image_tuple       = (64, 64, 3)
images_folder     = 'GZ1_Expert-ESRL_64x_tiff' 
images_subfolder  = 'GZ1_Expert-ESRL_partitions'
AE_weights_folder = 'CAE_weights_ESRL'

images_csv_train  = 'GZ1_Expert-ESRL_train_' + data_partition + '.csv'
images_csv_test   = 'GZ1_Expert-ESRL_test_' + data_partition + '.csv' 
images_csv_master = 'GZ1_Expert-ESRL.csv'  
...
labels_L_vector  = vectorize_labels_ERL(np.array(labels_L['ERL_EXPERT'], dtype=int))
labels_LA_vector = vectorize_labels_ERL(np.array(labels_LA['ERL_AMATEUR'], dtype=int))

# Ground-truth Expert labels:
labels_L_bin    = np.array(labels_L['ERL_EXPERT'], dtype=int) 
labels_U_bin    = np.array(labels_U['ERL_EXPERT'], dtype=int) 
labels_test_bin = np.array(images_labels_test['ERL_EXPERT'], dtype=int) 
labels          = (labels_L_bin, labels_U_bin, labels_test_bin) 

tag_pretraining = 'CzSL_Pre-training-Transformed_Expert-ERL'
tag_finetuning  = 'CzSL_Fine-tuning-Transformed_Expert-ERL'  
...
for execution in executions : 
    ...
    # (1) Pre-training Amateur:
    (CNN_train_images, CNN_train_labels, CNN_val_images,
     CNN_val_labels) = data_split_CNN_training(images_LA, labels_LA_vector, train_val_ratio=0.7) 
    
    AE_weights_filename = 'CAE_Expert-ESRL_FE_' + data_partition + '_encoder_weights.hdf5' 
    AE_weights_filename = os.path.join(CWD, AE_weights_folder, AE_weights_filename)
    
    CNN_pretraining = Convolutional_CNN(image_tuple, n_class=3, model_weights=AE_weights_filename)
    ...
    classification_performance_SSL(images_LA, predictions, labels, output_folder, tag_pretraining,
                                   train_time, int(data_partition), n_class=3)
    
    save_predictions_trin_SSL(labels_L, predictions[0], data_partition, 'L', output_folder, tag_pretraining)
    save_predictions_trin_SSL(labels_U, predictions[1], data_partition, 'U', output_folder, tag_pretraining)
    save_predictions_trin_SSL(images_labels_test, predictions[2], data_partition, 'test', output_folder,
                         tag_pretraining)
    ...
    config = (['EL', 'CS', 'EL_DEB[123]', 'CS_DEB[123]', 'CW', 'ACW', 'EDGE', 'DK', 'MG'], (8, 7, 5, 3))
    ...
    labels_train_vector = np.vstack((labels_L_vector, MLP_predictions)) 
    
    labels_U_MLP = MLP.predict_proba(MLP_scores_U)
    labels_U['MLP_E'] = labels_U_MLP[:, 0]
    labels_U['MLP_R'] = labels_U_MLP[:, 1] 
    labels_U['MLP_L'] = labels_U_MLP[:, 2] 
    
    # (2) Fine-tuning Expert:
    (CNN_train_images, CNN_train_labels, CNN_val_images,
     CNN_val_labels) = data_split_CNN_training(images_LA, labels_train_vector, train_val_ratio=0.7) 
    
    CNN_finetuning = Convolutional_CNN(image_tuple, n_class=3, model_weights=weights_filepath)
    ...
    classification_performance_SSL(images_LA, predictions, labels, output_folder, tag_finetuning,
                                   train_time, int(data_partition), n_class=3)
    
    save_predictions_trin_SSL(labels_L, predictions[0], data_partition, 'L', output_folder, tag_finetuning)
    save_predictions_trin_SSL(labels_U, predictions[1], data_partition, 'U', output_folder, tag_finetuning)
    save_predictions_trin_SSL(images_labels_test, predictions[2], data_partition, 'test', output_folder,
                         tag_finetuning) 

if data_partition == '5' : global_statistics_executions_SSL(CWD, tag_finetuning, trial_name, n_executions)
#===================================================================================================
