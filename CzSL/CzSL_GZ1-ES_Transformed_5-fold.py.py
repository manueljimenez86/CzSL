...
# Images and labels split for TR and TS sets: 
...
labels_L_vector  = vectorize_labels_bin(np.array(labels_L['ES_EXPERT'], dtype=int))
labels_LA_vector = vectorize_labels_bin(np.array(labels_LA['ES_AMATEUR'], dtype=int))

# Ground-truth Expert labels:
labels_L_bin    = np.array(labels_L['ES_EXPERT'], dtype=int) 
labels_U_bin    = np.array(labels_U['ES_EXPERT'], dtype=int) 
labels_test_bin = np.array(images_labels_test['ES_EXPERT'], dtype=int) 
labels          = (labels_L_bin, labels_U_bin, labels_test_bin) 

tag_pretraining = 'CzSL_Pre-training-Transformed_Expert-ES'
tag_finetuning  = 'CzSL_Fine-tuning-Transformed_Expert-ES'  
...
for execution in executions : 
    ...
    CNN_pretraining = Convolutional_CNN(image_tuple, n_classes=2, model_weights=AE_weights_filename)
    ...
    classification_performance_SSL(images_LA, predictions, labels, output_folder, tag_pretraining,
                                   train_time, int(data_partition))
    
    save_predictions_CNN(labels_L, predictions[0], data_partition, 'L', output_folder, tag_pretraining)
    save_predictions_CNN(labels_U, predictions[1], data_partition, 'U', output_folder, tag_pretraining)
    save_predictions_CNN(images_labels_test, predictions[2], data_partition, 'test', output_folder,
                         tag_pretraining)
    ...
    labels_U['MLP_EL'] = labels_U_MLP[:, 0]
    labels_U['MLP_CS'] = labels_U_MLP[:, 1] 
    
    # (2) Fine-tuning Expert:
    (CNN_train_images, CNN_train_labels, CNN_val_images,
     CNN_val_labels) = data_split_CNN_training(images_LA, labels_train_vector, train_val_ratio=0.7) 
    
    CNN_finetuning = Convolutional_CNN(image_tuple, n_classes=2, model_weights=weights_filepath)
    ...
    save_predictions_CNN(labels_L, predictions[0], data_partition, 'L', output_folder, tag_finetuning)
    save_predictions_CNN(labels_U, predictions[1], data_partition, 'U', output_folder, tag_finetuning)
    save_predictions_CNN(images_labels_test, predictions[2], data_partition, 'test', output_folder,
                         tag_finetuning) 
