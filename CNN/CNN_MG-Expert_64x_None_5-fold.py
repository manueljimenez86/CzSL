...
for execution in executions :
    ...
    for test_partition in partitions : 
        ...
        (images_train, labels_train, labels_train_vector, images_test, labels_test,
         labels_test_vector) = data_split_CNN_test(images_array, images_labels, 'MG_EXPERT', 
                                                test_partition, n_splits=5, n_class=2)
                                                   
        # Performing RUS/ROS:
        #(images_train, labels_train_vector) = Random_Under_Over_Sampling(images_train,
        #labels_train_vector, oversampling=False) 
        ...
        CNN = Convolutional_CNN(image_tuple, n_class=2, model_weights=None)
        ...
        classification_performance_imbalance(images_train, predictions, labels_test_vector, ratio,
                                             train_time, output_folder, trial_name, test_partition)
        
        save_predictions_MG(labels_test, predictions, test_partition, output_folder, trial_name)
 
       
global_statistics_executions_MG(CWD, trial_name, n_executions)
